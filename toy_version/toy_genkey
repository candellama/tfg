...
# Key Generation code for toy version of Dilithium signature scheme (ML-DSA)
...
class MLDSA:
    ...
    def set_parameters(self):
        ...
        self.q =  257 #8380417 modulus for coefficients
         ...
         # Toy signing bounds (used by sign/verify in toy_sign_gen.py)
    ...
    # principal function to generate the keypair 
    ...
    @staticmethod
    def _centered_mod(x: int, m: int) -> int:
        r = x % m
        return r - m if r > m // 2 else r

    def _poly_norm_inf(self, a: List[int]) -> int:
        return max(abs(MLDSA._center(x, self.q)) for x in a)
...
if __name__ == "__main__":
    ...
    rho_priv, s1, s2, t_priv = private_key
    
    print(f"Public Key rho (hex): {rho_pub.hex()}")
    print(f"Public Key t (first poly coeffs): {[t[0][:8]]}...")
